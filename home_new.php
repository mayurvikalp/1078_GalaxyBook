        ...
        <div style='float: left; width: 80%;'>
                <fieldset>
                    ...
                    <form id="form_update_status">
                        <textarea name="status" id="status" style="padding: 0px; margin: 0px; height: 50px; width: 84%; float: left;" onkeydown="checkForEnter();"></textarea>
                ...
                <fieldset>
                    ...
                    <?php
                        ...
                        $query = "(SELECT statuses.user, statuses.status AS content, statuses.type, statuses.time AS ts FROM statuses WHERE user IN($allfriends_IN))
UNION
(SELECT upload.username, upload.name, upload.type, upload.time FROM upload WHERE username IN($allfriends_IN))
ORDER BY ts DESC";
                        $result = mysql_query($query);
                        $counter = 0;
                        while ($row = mysql_fetch_array($result)) {
                            echo "<tr style='vertical-align:top;'><td><img src='profile_pics/thumbnails/" . getData($row['user'], "profile_pic") . "'/></td>";
                            echo "<td><strong><a style='font: Lucida Sans Unicode, Lucida Grande, sans-serif; ' href='profile.php?p=" . $row['user'] . "'>" . getData($row['user'], "first_name") . " " . getData($row['user'], "last_name") . "</a></strong><br>";
                            
                            if ($row['type'] == "status") 
                                echo "<span style='color: #333; font-size: 13px;'>" .$row['content'] . "</span></tr>";
                            else if (substr($row['type'],0,5) == "image") {
                                echo "<center>";
								echo "<a class='lightbox' href='pic/" . $row['content'] . "'>";
                                echo "<img src='image_resizer/image.php?width=350&height=450&image=" . $WEBSITE_DOCROOT . "pic/" . $row['content'] . "'/>";
                                echo "</a></center>";
                            }
                            $counter++;
                            if ($counter > 10) break;
